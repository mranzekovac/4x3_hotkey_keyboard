...
// Definiranje pinov od mikrokontrolerja
...
// Button helper class za zaznavo pritiska in izpustitve gumba
class button {
  public:
  const char key;
  const uint8_t pin;
...
void setup() { 
  // Varnostni pregled. Ozemljitveni zatič št. 1 (RX) za preklic vnosa s tipkovnice.
  pinMode(1, INPUT_PULLUP);
  if(!digitalRead(1)){
    failsafe();
  }
 
  // nastavi LED Off. Aktivira low.
